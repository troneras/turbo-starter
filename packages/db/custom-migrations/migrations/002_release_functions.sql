...
-- 1. Other OPEN releases (parallel work)
-- 2. Any deployed releases with higher deploy_seq than the candidate's branch point
...
BEGIN
    ...
    WITH release_entities AS (
        SELECT DISTINCT entity_id, entity_type, entity_key
        FROM entity_versions
        WHERE release_id = check_release_conflicts.release_id
    ),
...
RETURNS TRIGGER AS $$
BEGIN
  -- Set entity_name if not provided
  IF NEW.entity_name IS NULL THEN
    IF NEW.title IS NOT NULL THEN
      NEW.entity_name := NEW.title;
    ELSIF NEW.entity_key IS NOT NULL THEN
      NEW.entity_name := NEW.entity_key;
    ELSIF NEW.payload->>'name' IS NOT NULL THEN
      NEW.entity_name := NEW.payload->>'name';
    ELSE
      NEW.entity_name := NEW.entity_type || '_' || NEW.entity_id;
    END IF;
  END IF;
...
-- Useful for understanding release lineage and potential conflicts
